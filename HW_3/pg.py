...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    for i in range(n_layers):
      out = tf.contrib.layers.fully_connected(out, size) #activation function is tf.nn.relu by default
    out = tf.contrib.layers.fully_connected(out, output_size, activation_fn = output_activation)
...
class PG(object):
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(mlp_input = self.observation_placeholder,output_size = 1, scope = scope), axis = 1) 
    ...
    self.update_baseline_op = tf.train.AdamOptimizer(learning_rate = self.lr).minimize(baseline_loss)
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      path_returns = [0]*len(rewards)
      for t in range(len(rewards)):
        for i in range(t):
          path_returns[i] += (self.config.gamma**(t - i))*rewards[t]
      #######################################################
      #########          END YOUR CODE.          ############
  ...
  def calculate_advantage(self, returns, observations):
    ...
    #########   YOUR CODE HERE - 5-10 lines.   ############
    if self.config.use_baseline:
      adv = returns - self.sess.run(self.baseline, feed_dict = {self.observation_placeholder : observations})[0]
