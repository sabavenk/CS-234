...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, shape = (None, state_shape[0], state_shape[1], self.config.state_history))
        self.a = tf.placeholder(tf.int32, shape = (None,))
        self.r = tf.placeholder(tf.float32, shape = (None,))        
        self.sp = tf.placeholder(tf.uint8, shape = (None, state_shape[0], state_shape[1], self.config.state_history))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse = resue) as _:
            out = layers.fully_connected(inputs = tf.contrib.layers.flatten(state, scope), num_outputs = num_actions, activation_fn = None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_target = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = target_q_scope)
        q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = q_scope)
        q_temp = []
        for i in range(len(q_target)):
            q_temp.append(tf.assign(q_target[i], target[i]))
        self.update_target_op = tf.group(*q_temp)
    ...
    def add_loss_op(self, q, target_q):
        ...
        non_terminal = 1 - tf.cast(self.done_mask, tf.float32)
        index = tf.one_hot(self.a, num_actions)
        q_samp = self.r + non_terminal*self.config.gamma*tf.reduce_max(target_q, axis = 1)
        q_curr = tf.reduce_sum(q*index, axis = 1)
        self.loss = tf.reduce_mean((q_samp - q_curr)**2)
    ...
    def add_optimizer_op(self, scope):
        ...
        #define variables, gradients, and optimizer

        var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = scope)
        optimizer = tf.train.AdamOptimizer(learning_rate = self.lr)
        grad, vec = list(zip(*optimizer.compute_gradient(self.loss, var)))

        # Clip gradients

        if self.config.grad_clip:
            grad, _ = tf.clip_by_norm(gradients, self.config.clip_val)

        # Optimize using clipped gradients

        self.grad_norm = tf.global_norm(grad)
        self.train_op = optimizer.apply_gradients(list(zip(grad, vec))
